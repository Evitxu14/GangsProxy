...
public class KillAuraCommand extends Command {
        ...
        return command("killAura")
            .then(argument("toggle", toggle()).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("Aura Asesina ", "Kill Aura ") + toggleStrCaps(CONFIG.client.extra.killAura.enabled));
            }))
            .then(literal("attackDelay").then(argument("ticks", time(0, 1000)).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("¡Ticks de Retraso de Ataque Establecidos!", "Attack Delay Ticks Set!"));
            })))
            .then(literal("tpsSync").then(argument("toggle", toggle()).executes(c -> {
                CONFIG.client.extra.killAura.tpsSync = getToggle(c, "toggle");
                c.getSource().getEmbed()
                    .title(Lang.t("Sincronización TPS ", "TPS Sync ") + toggleStrCaps(CONFIG.client.extra.killAura.tpsSync));
            })))
            .then(literal("targetPlayers").then(argument("toggle", toggle()).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("Jugadores Objetivo ", "Target Players ") + toggleStrCaps(CONFIG.client.extra.killAura.targetPlayers));
            })))
            .then(literal("targetHostileMobs")
                .then(literal("onlyAggressive").then(argument("toggle", toggle()).executes(c -> {
                    ...
                    c.getSource().getEmbed()
                        .title(Lang.t("Solo Mobs Hostiles Agresivos ", "Target Hostile Mobs Only Aggressive ") + toggleStrCaps(CONFIG.client.extra.killAura.onlyHostileAggressive));
                })))
                .then(argument("toggle", toggle()).executes(c -> {
                    CONFIG.client.extra.killAura.targetHostileMobs = getToggle(c, "toggle");
                    c.getSource().getEmbed()
                        .title(Lang.t("Mobs Objetivo ", "Target Mobs ") + toggleStrCaps(CONFIG.client.extra.killAura.targetHostileMobs));
                })))
            .then(literal("targetNeutralMobs")
                .then(argument("toggle", toggle()).executes(c -> {
                    CONFIG.client.extra.killAura.targetNeutralMobs = getToggle(c, "toggle");
                    c.getSource().getEmbed()
                        .title(Lang.t("Mobs Neutrales Objetivo ", "Target Neutral Mobs ") + toggleStrCaps(CONFIG.client.extra.killAura.targetNeutralMobs));
                }))
                .then(literal("onlyAggressive")
                    .then(argument("toggle", toggle()).executes(c -> {
                        CONFIG.client.extra.killAura.onlyNeutralAggressive = getToggle(c, "toggle");
                        c.getSource().getEmbed()
                            .title(Lang.t("Solo Mobs Neutrales Agresivos ", "Target Neutral Mobs Only Aggressive ") + toggleStrCaps(CONFIG.client.extra.killAura.onlyNeutralAggressive));
                    }))))
            .then(literal("weaponSwitch").then(argument("toggle", toggle()).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("Cambio de Arma ", "Weapon Switching ") + toggleStrCaps(CONFIG.client.extra.killAura.switchWeapon));
            })))
            .then(literal("weaponType").then(argument("type", enumStrings(WeaponType.values())).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("Tipo de Arma Establecido", "Weapon Type Set"));
            })))
            .then(literal("weaponMaterial").then(argument("material", enumStrings(WeaponMaterial.values())).executes(c -> {
                ...
                c.getSource().getEmbed()
                    .title(Lang.t("Material de Arma Establecido", "Weapon Material Set"));
            })))
            .then(literal("targetCustom")
                .then(argument("toggle", toggle()).executes(c -> {
                    CONFIG.client.extra.killAura.targetCustom = getToggle(c, "toggle");
                    c.getSource().getEmbed()
                        .title(Lang.t("Objetivo Personalizado ", "Target Custom ") + toggleStrCaps(CONFIG.client.extra.killAura.targetCustom));
                }))
                .then(literal("add").then(argument("entityType", entity()).executes(c -> {
                    ...
                    c.getSource().getEmbed()
                        .title(Lang.t("Añadido ", "Added ") + type);
                })))
                .then(literal("del").then(argument("entityType", entity()).executes(c -> {
                    ...
                    c.getSource().getEmbed()
                        .title(Lang.t("Eliminado ", "Removed ") + type);
                }))))
            .then(literal("raycast").then(argument("toggle", toggle()).executes(c -> {
                CONFIG.client.extra.killAura.raycast = getToggle(c, "toggle");
                c.getSource().getEmbed()
                    .title(Lang.t("Raycast ", "Raycast ") + toggleStrCaps(CONFIG.client.extra.killAura.raycast));
            })))
            .then(literal("priority")
                .then(literal("none").executes(c -> {
                    ...
                    c.getSource().getEmbed()
                        .title(Lang.t("Prioridad Establecida", "Priority Set"));
                }))
    ...
    public void defaultEmbed(Embed builder) {
        builder
            .addField(Lang.t("Aura Asesina", "KillAura"), toggleStr(CONFIG.client.extra.killAura.enabled))
            .addField(Lang.t("Jugadores Objetivo", "Target Players"), toggleStr(CONFIG.client.extra.killAura.targetPlayers))
            .addField(Lang.t("Mobs Hostiles Objetivo", "Target Hostile Mobs"), toggleStr(CONFIG.client.extra.killAura.targetHostileMobs) + " [onlyAggressive: " + toggleStr(CONFIG.client.extra.killAura.onlyHostileAggressive) + "]")
            .addField(Lang.t("Mobs Neutrales Objetivo", "Target Neutral Mobs"), toggleStr(CONFIG.client.extra.killAura.targetNeutralMobs) + " [onlyAggressive: " + toggleStr(CONFIG.client.extra.killAura.onlyNeutralAggressive) + "]")
            .addField(Lang.t("Objetivo Personalizado", "Target Custom"), toggleStr(CONFIG.client.extra.killAura.targetCustom))
            .addField(Lang.t("Cambio de Arma", "Weapon Switching"), toggleStr(CONFIG.client.extra.killAura.switchWeapon))
            .addField(Lang.t("Tipo de Arma", "Weapon Type"), CONFIG.client.extra.killAura.weaponType.name().toLowerCase())
            .addField(Lang.t("Material de Arma", "Weapon Material"), CONFIG.client.extra.killAura.weaponMaterial.name().toLowerCase())
            .addField(Lang.t("Ticks de Retraso de Ataque", "Attack Delay Ticks"), CONFIG.client.extra.killAura.attackDelayTicks)
            .addField(Lang.t("Sincronización TPS", "TPS Sync"), toggleStr(CONFIG.client.extra.killAura.tpsSync))
            .addField(Lang.t("Raycast", "Raycast"), toggleStr(CONFIG.client.extra.killAura.raycast))
            .addField(Lang.t("Prioridad", "Priority"), CONFIG.client.extra.killAura.priority.name().toLowerCase())
            .primaryColor();
